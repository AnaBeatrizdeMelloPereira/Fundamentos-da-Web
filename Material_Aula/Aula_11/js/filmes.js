...
const cartazes = document.querySelector("#cartazes")

/*for(let i=0;i<filmes.length; i++) {
    const filme = filmes[i]
    let divElem = `<div class="cartaz">
    <h1>${filme.titulo}</h1>
    <p>${filme.resumo}</p>
    <img src="${filme.figura}">
</div>`
    cartazes.innerHTML += divElem
}*/
...
/*const criaGeneroLi = (elementoUl, generoList) =>{
    generoList.forEach(function(genero){
        ...
        //criando o texto que será incluido
        const generoTN = document.createTextNode(genero)
        //inserindo o genero
        LiHTML.appendChild(generoTN)

        elementoUl.appendChild(LiHTML)
    })
}*/

const criaLi = (elementoUl, List) =>{
    List.forEach(function(item){
        const LiHTML = document.createElement("li")
        const textoTN = document.createTextNode(item)
        LiHTML.appendChild(textoTN)
...
const criaCartaz = filme => {
    ...
    const cartazHTML = document.createElement("div")
    const tituloHTML = document.createElement("h1")
    const resumoHTML = document.createElement("p")
    const imgHTML = document.createElement("img")
    const generoListHTML = document.createElement("ul")
    const elementoListHTML = document.createElement("ul")
    
    //adicionando classes e atributos aos elementos criados
    cartazHTML.classList.add("cartaz")
    generoListHTML.classList.add("genero")
    elementoListHTML.classList.add("elenco")

    criaLi(generoListHTML, filme.generos)
    criaLi(elementoListHTML, filme.elenco)

    imgHTML.src = filme.figura
    imgHTML.alt = filme.titulo

    //criando nós textuais
    const tituloTN = document.createTextNode(filme.titulo)
    const resumoTN = document.createTextNode(filme.resumo)

    //adicionando os nós textuais aos elementos
    tituloHTML.appendChild(tituloTN)
    resumoHTML.appendChild(resumoTN)

    //adicionando os filhos do elemento principal
    cartazHTML.appendChild(tituloHTML)
    cartazHTML.appendChild(resumoHTML)
    cartazHTML.appendChild(imgHTML)
    cartazHTML.appendChild(generoListHTML)
    cartazHTML.appendChild(elementoListHTML)
...
filmes.forEach(function(filme){
    cartazes.appendChild(criaCartaz(filme))
})

/**
 * Funções auxiliares de arrays!
 * forEach(function(element, indice*, funcao de callback*))
 */